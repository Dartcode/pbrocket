...
Procedure.d RKT_audio_get_row(*this.audio_cb)
  Protected pos.q = BASS_ChannelGetPosition(*this\stream, #BASS_POS_BYTE)
        
  CompilerIf #RKT_SYNC_PLAYER
    ProcedureReturn BASS_ChannelBytes2Seconds(*this\stream, pos) * *this\rowrate
  CompilerElse
    ; + 0.05 => just a little rounding to match exact editor position
    ProcedureReturn BASS_ChannelBytes2Seconds(*this\stream, pos) * *this\rowrate  + 0.05
  CompilerEndIf
EndProcedure

Procedure RKT_audio_set_row(*this.audio_cb, row.i)
  Protected pos.q = BASS_ChannelSeconds2Bytes(*this\stream, row / *this\rowrate)
...
Procedure RKT_audio_rows(*this.audio_cb)
  Protected len.q = BASS_ChannelGetLength(*this\stream, #BASS_POS_BYTE)
  ProcedureReturn Round(BASS_ChannelBytes2Seconds(*this\stream, len), #PB_Round_Up) * *this\rowrate
EndProcedure
...
CompilerIf Not #RKT_SYNC_PLAYER
  ...
  Procedure.i RKT_update(*this.sync_device, row.i, *cb.audio_cb)
      ...
      Select NetEvent
        ...
        Case #PB_NetworkEvent_Disconnect
          ...
          *this\isConnected = #False
          MessageRequester("PB Rocket", "Rocket Editor is gone!")
          End
      EndSelect
    EndIf
    ...
    If CurrentCommand = #RKT_HANDSHAKE And (InWritePtr - InReadPtr >= Len(#RKT_EDITOR_GREETING) -1)
      ;Debug PeekS(*this\inBuffer, Len(#RKT_EDITOR_GREETING), #PB_Any)
  ...
  Procedure RKT_sync_connect(*this.sync_device)
    Protected loop.i = 0
